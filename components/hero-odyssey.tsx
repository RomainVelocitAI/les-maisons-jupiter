  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
          ...
          float dist = abs(uv.x);
          // Compute base color using hue - Enhanced for golden lightning
          vec3 baseColor = hsv2rgb(vec3(uHue / 360.0, 0.85, 0.95));
          // Compute color with intensity and speed affecting time - Brighter for gold
          vec3 col = baseColor * pow(mix(0.0, 0.12, hash11(iTime * uSpeed)) / dist, 1.0) * uIntensity;
...
export const HeroSection: React.FC = () => {
  ...
  // State for the lightning hue - Golden lightning
  const [lightningHue, setLightningHue] = useState(45); // Golden hue for "éclair d'or"
  ...
  return (
    <div className="relative w-full bg-black text-white overflow-hidden">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 h-screen flex items-center justify-center">
        ...
        >
          ...
          <motion.h2
            variants={itemVariants}
            className="text-3xl md:text-5xl font-light bg-gradient-to-r from-yellow-200 via-yellow-300 to-amber-400 bg-clip-text text-transparent"
          >
      ...
      >
        ...
        {/* Central light beam - Golden lightning */}
        <div className="absolute top-0 w-[100%] left-1/2 transform -translate-x-1/2 h-full">
          <Lightning
            hue={lightningHue} // Golden lightning
            xOffset={0}
            speed={1.6}
            intensity={0.8}
            size={2}
